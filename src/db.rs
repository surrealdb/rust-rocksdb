...
impl<T: ThreadMode, D: DBInner> DBCommon<T, D> {
    ...
    /// Return the value and matched timestamp associated with a key in the given column family
    /// with read options. Uses the `rocksdb_get_cf_with_ts` FFI function.
    ///
    /// Returns a tuple of `(value, matched_timestamp)` where either may be `None` if the key
    /// is not found.
    ...
    pub fn get_cf_with_ts<K: AsRef<[u8]>>(
        ...
        self.get_cf_with_ts_opt(cf, key.as_ref(), &ReadOptions::default())
    }
